...
def _evaluate_case(case: dict[str, Any], result: dict[str, Any], elapsed_ms: int) -> dict[str, Any]:
    ...
    if case.get("expect_no_tools") and tools_used:
        failures.append(f"expected no tools, saw {tools_used}")
...
def _print_case_report(report: dict[str, Any]) -> None:
...
def main() -> None:
    ...
    reports = asyncio.run(_run_eval(selected_cases, strict=args.strict))
    if args.json_out:
